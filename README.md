...
📡 類股強弱排名
🔥 CPO (3檔) 5日均漲+4.2% | 保守型BUY 1 / 積極型BUY 0 / WATCH 1
📈 機器人 (2檔) 5日均漲+1.5% | 保守型BUY 0 / 積極型BUY 1 / WATCH 0
📉 重電 (2檔) 5日均漲-0.8% | 保守型BUY 0 / 積極型BUY 0 / WATCH 1
```
...
（積極型BUY、WATCH 與操作建議會在後續訊息分開提供）
```

> 通知預設分五則推送：市場總覽 → 策略摘要 → 保守型BUY → 積極型BUY → WATCH與操作建議。
...
```
signal_score = 基本面 × 20% + 技術面 × 50% + 回測勝率 × 30%（排序用途）
```

| 分類 | 動作 | 核心條件 |
|--------|------|------|
| 🔵 保守型BUY | BUY | 站穩月季線 + 20日趨勢向上 + 位置合理 |
| 🟠 積極型BUY | BUY | 轉強或接近突破 + 量價配合 |
| 🟡 WATCH | WATCH | 方向可能不差，但趨勢/位置/回測仍有疑慮 |
| ⚪ SKIP | SKIP | 目前不適合布局或驗證不足 |
...
| **RSI** | 30 < RSI < 70（20 分） | 其他區間 0 分 |

### 歷史回測

對過去 2 年所有技術分 ≥ 60 的交易日，模擬：
- 以**收盤價進場**
- 持有 **10 個交易日**
- **+10% 停利** / **-8% 停損** / 到期以收盤結算
...
```python
CONFIG = {
    "eps_threshold": 2.0,       # EPS 門檻
    "roe_threshold": 15.0,      # ROE 門檻
    "backtest_years": 2,        # 回測年數
...
signal_score = round(
    0.2 * fund_score +   # 基本面 20%
    0.5 * tech_score +   # 技術面 50%（目前權重最高）
    0.3 * bt_score,      # 回測 30%
    1
)
